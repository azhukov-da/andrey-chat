...
- **Line coverage** — [tools/coverage-gate](tools/coverage-gate/) compares backend and frontend line coverage against 80%. It currently **reports** the shortfall without failing; `test.bat` calls it without `--enforce` because the frontend layer is still far short, and failing on coverage would bury real test failures. Turning the gate on is adding that one flag.

### Where the uncovered scenarios went, and where the spec and the code disagree

Two standing documents answer the questions the coverage report raises but cannot itself answer:

- [docs/test-layer-triage.md](docs/test-layer-triage.md) — every specified scenario, assigned to the layer that observes it most directly, with a reason per row. This is the answer to "why does the backend not cover X". It is generated by [tools/layer-triage](tools/layer-triage/) from the specs and `assignments.json`; edit the assignments and re-run rather than editing the document.
- [docs/spec-gaps.md](docs/spec-gaps.md) — the places where the implementation diverges from a scenario, found by writing the test the specification describes and watching it fail. Each row names the expected behaviour, the observed behaviour, and the source location that produces it; the corresponding test keeps its spec-accurate assertions and is marked `Skip`, so the coverage report shows the scenario as claimed-but-not-passing rather than quietly blessing the divergence.
